...
class Jacobian_Master:
    ...
    def zip_data_dir(self):
        # function to move the current bgaPEST files to the data directory and zip it up for Condor
        for file in glob.glob('bgaPEST.#*'):
            shutil.move(os.path.join(os.getcwd(),file),os.path.join(os.getcwd(),'data',file))
        print 'moved current bgaPEST files into data subdirectory'
        # delete the existing zip file to avoid conflicts
        if os.path.exists('data.zip'):
            os.remove('data.zip')
        
        # now zip up the data directory, maintaining the directory structure
        dir_name = 'data'
        currd = os.path.join(os.getcwd(),dir_name)
        czip = zipfile.ZipFile('data.zip','w')
        for root,dir,files in os.walk(currd):
            for f in files:
                fname = os.path.join(root,f)
                new_path = os.path.normpath(fname.replace(currd,dir_name))
                czip.write(fname,new_path,zipfile.ZIP_DEFLATED)
        czip.close() 
        print 'zipped up the data directory'
    ...
    def jacobian_master(self):
        ...
        if os.path.exists(os.path.join(os.getcwd(),'log')):
            print 'removing old log files from log subdirectory'
...
class Jacobian_one_run:
    ...
    def make_model_input_and_run(self):
        ...
        if self.perturb < self.NPAR:
            ...
            pertamt = self.derinc[pertgrp]
            print 'unperturbed par val => %f' %(self.parvals[self.perturb])
            self.parvals[self.perturb] += self.parvals[self.perturb]*pertamt
            print 'perturbed par val => %f' %(self.parvals[self.perturb])
